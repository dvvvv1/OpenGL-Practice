...
vec3 PhongLight(in vec3 LightPosition_worldspace1, in vec3 LightDirection_cameraspace1, in vec3 Position_worldspace1, in vec3 Normal_cameraspace1, in vec3 EyeDirection_cameraspace1,
                in vec3 light_ambient, in vec3 light_diffuse, in vec3 light_specular, in vec3 mtl_diffuse1, in vec3 mtl_ambient1, in vec3 mtl_specular1)
                {
                    ...
                    MaterialDiffuseColor * light_diffuse * cosTheta / (distance*18) +
                    // Specular : reflective highlight, like a mirror
                    MaterialSpecularColor * light_specular * pow(cosAlpha, shininess) / (distance*18) ;
                }
...
void main(void) {
        //flip texcoord to match the model       
     	...
     	if(light_mode == 0)
     	{    	          	 
     	        vec3 totLight = vec3(0.0f,0.0f,0.0f);
     	        for(int i = 0; i < 24; i++)
     	        {    	           
     	        // lamp 1 light 1
     	        ...
     	        vec3 light1 =  PhongLight(LightPosition_worldspace[i], LightDirection_cameraspace1, Position_worldspace, Normal_cameraspace, EyeDirection_cameraspace, light_ambient[i], light_diffuse[i], light_specular[i], mtl_diffuse, mtl_ambient, mtl_specular);
     	...
     	else if(light_mode == 1)
     	{
     	    // lamp 1 light 1
     	        ...
     	        fragColour = vec4(fragColour.xyz,1.0)*vec4(1.0,1.0,1.0,1.0);
     	        fragColour.a = 0.8;
     	}
