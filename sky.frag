...
vec3 PhongLight(in vec3 LightPosition_worldspace1, in vec3 LightDirection_cameraspace1, in vec3 Position_worldspace1, in vec3 TextureNormal_tangentspace1, in vec3 EyeDirection_cameraspace1,
                in vec3 light_ambient, in vec3 light_diffuse, in vec3 light_specular, in vec3 mtl_diffuse1, in vec3 mtl_ambient1, in vec3 mtl_specular1)
                {
                    ...
                    MaterialDiffuseColor * light_diffuse * cosTheta / (distance*0.3) +
                    // Specular : reflective highlight, like a mirror
                    MaterialSpecularColor * light_specular * pow(cosAlpha, shininess) / (distance*0.3) ;
                }
...
void main(void) {
            ...
            if(light_mode == 0)
            {
                // lamp 1 light 1
     	        ...
     	        vec3 light1 =  PhongLight(LightPosition_worldspace[0], LightDirection_cameraspace1, Position_worldspace, TextureNormal_tangentspace, EyeDirection_cameraspace, light_ambient[0], vec3(1.0,1.0,1.0)+light_diffuse[0], vec3(1.0,1.0,1.0)+light_specular[0], MaterialDiffuseColor, MaterialAmbientColor, MaterialSpecularColor);
                ...
                // MIRROR stuff
     	        ...
     	        fragColour = texture(texMap1, flip_texCoord );
            }
